            ...
            if (userId) {
                ...
                if (user) {
                    ...
                    try {
                        ...
                        setPayments([
                            buildPaymentObject(user, userIdValue, paymentProof),
                        ]);
                    } catch (err) {
                        console.error("Error fetching payment proof:", err);
                        setPayments([
                            buildPaymentObject(user, userIdValue),
                        ]);
                    }
                }
            } else {
                ...
                const paymentsData = users.map((user: User) =>
                    buildPaymentObject(user, user._id || (user as any).id || "")
                );
...
function buildPaymentObject(user: User, id: string, paymentProof?: any): PaymentData {
    return {
        userId: id,
        username: user.username,
        email: user.email,
